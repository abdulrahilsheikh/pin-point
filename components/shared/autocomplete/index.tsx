...
import { ScrollViewListItem } from "./scroll-into-view";
import { theme } from "./theme";
import { debounce } from "@/utils/helpers";
import { AutocompleteDropdownItem, IAutocompleteDropdownProps } from "./types";
import { AutocompleteDropdownContext } from "./context/dropdown-context";
...
>(
  forwardRef((props: IAutocompleteDropdownProps, ref) => {
    const {
      ...
      value,
      setValue,
      showValueAfterSelect = true,
      emptyResultCompWithAction,
    } = props;
    ...
    const selectedItem = value != undefined ? value : localSelectedItem;
    const setSelectedItem =
      setValue != undefined ? setValue : setLocalSelectedItem;
    ...
    useEffect(() => {
      if (selectedItem && showValueAfterSelect) {
        setInputValue(selectedItem.title ?? "");
      } else {
        setInputValue("");
      }
    }, [selectedItem, showValueAfterSelect]);
    ...
    useEffect(() => {
      ...
      if (
        ...
        showValueAfterSelect
      ) {
        setInputValue(selectedItem.title || "");
      }
    }, [isOpened, loading, searchText, selectedItem, showValueAfterSelect]);
    ...
    const ListEmptyComponent = useMemo(() => {
      return (
        emptyResultCompWithAction?.(inputValue, () => {
          setIsOpened(false);
          setInputValue("");
          closeAll();
        }) ??
        EmptyResultComponent ?? (
          <ThemedView>
            <ThemedText>{emptyResultText}</ThemedText>
          </ThemedView>
        )
      );
    }, [
